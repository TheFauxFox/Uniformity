...
public class MobNameplates extends Module {
    ...
    public void render(Entity entity, MatrixStack matrices, VertexConsumerProvider vertexConsumers, Quaternionf cameraRotation) {
        ...
        matrices.push();
        matrices.scale(-0.0267F, -0.0267F, 0.0267F);
        ...
        mc.textRenderer.draw(name, 0, 0, 0xFFFFFF, false, matrix, vertexConsumers, TextRenderer.TextLayerType.SEE_THROUGH, 0, 0xF000F0);
        matrices.pop();
        ...
        mc.textRenderer.draw(healthFormat.format(livingEntity.getHealth()), 2, 14, 0xFFFFFF, false, matrix, vertexConsumers, TextRenderer.TextLayerType.SEE_THROUGH, 0, 0xF000F0);
        String maxHpStr = Formatting.BOLD + healthFormat.format(livingEntity.getMaxHealth());
        mc.textRenderer.draw(maxHpStr, (int) (halfSize / healthValueTextScale * 2) - mc.textRenderer.getWidth(maxHpStr) - 2, 14, 0xFFFFFF, false, matrix, vertexConsumers, TextRenderer.TextLayerType.SEE_THROUGH, 0, 0xF000F0);
        String percStr = (int) (100 * livingEntity.getHealth() / livingEntity.getMaxHealth()) + "%";
        mc.textRenderer.draw(percStr, (int) (halfSize / healthValueTextScale) - mc.textRenderer.getWidth(percStr) / 2.0F, 14, 0xFFFFFF, false, matrix, vertexConsumers, TextRenderer.TextLayerType.SEE_THROUGH, 0, 0xF000F0);
        matrices.pop();
